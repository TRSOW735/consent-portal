...
export async function GET(_: Request, ctx: { params: Promise<{ reference: string }> }) {
  ...
  if (pr.status === "paid") {
    return NextResponse.json({ status: "paid", signature: pr.signature });
  }

  const org = pr.orgId ? await prisma.organization.findUnique({ where: { id: pr.orgId } }) : null;

  const rpc =
    (org?.rpcUrl && org.rpcUrl.length > 0 ? org.rpcUrl : "") ||
